    ...
    pub(crate) async fn connect<R: Runtime>(
            ...
            .0
        {
            ...
            "sqlite" => {
                ...
                // This plugin's public IPC API (execute/select) has no way for
                // JS-side callers to pin multiple statements to the same
                // physical connection — every call independently acquires and
                // releases a connection from the pool. That silently breaks
                // any multi-statement BEGIN/.../COMMIT transaction issued from
                // JS (statements can land on different connections), and can
                // leave a stray open transaction holding SQLite's write lock
                // on a connection that then gets handed to an unrelated
                // caller, causing long stalls or "database is locked" errors.
                // SQLite only safely supports one writer at a time anyway, so
                // capping the pool at exactly one connection isn't a real
                // concurrency loss — it makes every call serialize onto that
                // same connection, restoring real transactional semantics.
                // connect_with (not a bare PRAGMA exec after connecting) so
                // these apply to every connection the pool ever opens,
                // including ones it re-opens after an idle timeout.
                let connect_options = sqlx::sqlite::SqliteConnectOptions::from_str(conn_url)?
