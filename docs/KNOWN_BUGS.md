...
Two accessibility fix passes converted the highest-traffic click-only `<div>`/`<span>` elements (POS actions, then ~20 more list rows and 5 hand-rolled comboboxes) to real buttons/`role="button"` with keyboard support. Seven named, lower-traffic sites remain, same defect class:
- `components/products/catalog-list.tsx:214` — catalog row click-to-open
- `components/dashboard/dashboard-recent-activity.tsx:115` — activity row
- `components/pos/pos-customer-selector.tsx:105,209,246` — a 6th hand-rolled combobox (POS)
- `components/dashboard/sync-indicator.tsx:283,313` — sync panel rows
- `components/setup/steps/backup-step.tsx:64` — setup wizard option
- `components/pos/pos-cart.tsx:267` — clickable `<span>`
- `components/pos/transaction-item.tsx:125` — transaction row

Everything else the original audit flagged is now fixed (skip link, `<th scope>`, contrast, `<h1>` on every page except `procurement/{vendors,requests,new,edit}/page.tsx`, which still lack one).
