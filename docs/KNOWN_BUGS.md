...
### L7. `EnforcesStaffOwnership::storeIdBelongsToCaller(null)` returns `true`, letting `update()` orphan a staff row to `store_id = NULL`
- **Category:** Bug / Data Integrity — **Confirmed** (found by an independent Opus review pass of the C1 fix, 2026-09-24) — **Not a regression**: pre-fix `update()` had no store_id check at all, so this is new-but-not-worse behavior.
- **File:** `laravel-server/app/Http/Controllers/Concerns/EnforcesStaffOwnership.php`, `StaffController::update()`

`store_id => 'nullable|exists:stores,id'` plus `ConvertEmptyStringsToNull` means a client sending `store_id: ""` writes a literal `NULL`. `storeIdBelongsToCaller()` early-returns `true` for `null` (needed so editing the owner's own null-store_id "Main Account" row keeps working). But applied to an actual *staff* row, this nulls out `store_id` legitimately-but-silently: the row becomes invisible to `visibleStaffBaseQuery()` for everyone except `super_admin` (matches neither `whereIn('store_id', $storeIds)` nor `id = $owner->id`), and `ScopesToTenant::tenantOwnerId()` no longer resolves it to any real tenant. The staff member can still log in but the owner can no longer see or repair the row via the normal staff UI. **Fix:** distinguish "this row's `store_id` is already null" (harmless no-op, current behavior is fine) from "this call is actively nulling a previously-set `store_id`" (should probably be rejected, or at least requires `super_admin`) — needs a product decision on the null-out use case's legitimacy first.

### L8. `release.yml`'s workflow-wide `concurrency: {group: release}` can silently drop an entire release if two tags are pushed close together
- **Category:** Reliability — **Confirmed** (found by an independent Opus review pass of the M7 concurrency-guard fix, 2026-09-24)
- **File:** `.github/workflows/release.yml`

GitHub Actions keeps at most one *pending* run per concurrency group; a third run arriving cancels the older *pending* one outright (not the currently-running one, since `cancel-in-progress: false`). The group here is `release` (not scoped by tag, deliberately — see the fix's own comment about the shared `/downloads.dumosrx.com/` root path). Pushing `v1.0.0`, `v1.0.1`, `v1.0.2` in quick succession means `v1.0.1`'s entire run — its binaries, its `updater.json` entry — is cancelled and never built, with no failure signal anywhere. For the four `deploy-*` workflows "newest wins" is the correct semantic (redeploying the same target repeatedly); for a versioned release it's silent data loss. **Fix:** either scope the group to `${{ github.ref }}` (loses the shared-root-path protection the fix was for) or move that specific protection down to just the "FTP Deploy Root Files" step (e.g. a separate job with its own narrow concurrency group) instead of the whole workflow.

### L9. Legacy-row store_id claim (fixed for atomicity, M6) still never reaches the sync queue
- **Category:** Bug / Data Integrity — **Confirmed**, pre-existing (not introduced by M6's fix) — found by an independent Opus review pass of the M6 fix, 2026-09-24
- **File:** `client/lib/db/base-helpers.ts` (`update()`/`softDelete()`'s `writeUpdate`/`writeSoftDelete`)

The claim statement (`UPDATE {table} SET store_id = ? WHERE id = ?`) doesn't set `_synced = 0`, and the `addToSyncQueue()` call right after it is built from the caller's own `data`/`record`, which never includes the newly-claimed `store_id`. So the claim commits locally (correctly, atomically with the edit as of M6's fix) but the server never learns the row now belongs to this store — a second device pulling this row later still sees it as `store_id NULL` server-side. Same gap existed before M6 (the old bare-statement claim had it too); M6 fixed the atomicity, not this. **Fix:** either fold `store_id` into the queued `UPDATE` payload when a claim occurs, or queue a small separate sync entry for the claim.
