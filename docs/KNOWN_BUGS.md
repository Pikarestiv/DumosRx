...
> **M10 mostly fixed 2026-09-24** — see `docs/FIXED_BUGS.md`. `sonner`, `tailwind-merge`, `eslint`, and every drifted `@radix-ui/*` package are now aligned to the same major version (or newer) across `client/` and `web/`. `next` itself is the one deliberate exception, tracked below as **M10-remainder**.

### M10-remainder. `client/` and `web/` remain on different Next.js majors (15 vs 16)
- **Category:** Architecture — **Confirmed**, deliberate for now
- **File:** `client/package.json` (`next@15.5.26`), `web/package.json` (`next@16.3.6`)

Unlike `sonner`/`tailwind-merge`/`eslint`/`@radix-ui/*` (all now aligned — see `docs/FIXED_BUGS.md`), `next` itself was deliberately left on different majors: `client/` is a Tauri-wrapped static export with its own build/native-shell constraints, and jumping it to Next 16 is a materially bigger, riskier change than a patch-level CVE fix (H3 already bumped both apps' `next` within their existing majors specifically to avoid conflating a security fix with a major-version migration). **Fix:** schedule the `client/` → Next 16 migration as its own project once the `web/` → `client/` dashboard port (the actual reason version parity matters) is further along, not as a quick dependency bump.
...
- **Dependency drift between `client/` and `web/` (M10)** was a natural consequence of the two apps evolving independently before the current migration effort began. Now mostly resolved (`sonner`/`tailwind-merge`/`eslint`/`@radix-ui/*` all aligned, 2026-09-24) — the one remaining major-version gap (`next` 15 vs 16, **M10-remainder**) is deliberate, not oversight, and belongs with the `web/`→`client/` dashboard migration's own scope rather than a standalone task.
...
6. ~~Treat the client/web dependency-drift (M10)~~ — **mostly done 2026-09-24**: `sonner`/`tailwind-merge`/`eslint`/`@radix-ui/*` aligned; `next`'s major-version gap (**M10-remainder**) and the multi-tab data-loss risk (**C5**) remain scoped mini-projects with their own design pass, not quick patches — both require an actual decision (Next 16 migration timing; single-writer-tab architecture) rather than a local code change. **M14-remainder** (`laravel/framework` 11→12) belongs in this same "needs its own review pass" category — it's a real framework-upgrade project now, not a dependency patch.
...
3. **C5** (multi-tab data loss), **M10-remainder/M11** (Next.js 15→16 migration for `client/`, localStorage-token architecture), and **M14-remainder** (Laravel 11→12 upgrade) — schedule as their own design/upgrade projects; not blocking for the above, but shouldn't be indefinitely deferred given C5's silent-data-loss nature.
