...
(),
  medicine_id UUID NOT NULL REFERENCES medicines
(id) ON
DELETE CASCADE,
...
-- Create indexes
CREATE INDEX
IF NOT EXISTS idx_inventory_medicine ON inventory
(medicine_id);
CREATE INDEX
IF NOT EXISTS idx_inventory_batch ON inventory
(batch_number);
CREATE INDEX
IF NOT EXISTS idx_inventory_expiry ON inventory
(expiry_date);
CREATE INDEX
IF NOT EXISTS idx_inventory_status ON inventory
(status);
CREATE INDEX
IF NOT EXISTS idx_inventory_stock_level ON inventory
(quantity_in_stock);
...
FROM inventories i
  JOIN medicines m ON i.medicine_id = m.id
WHERE i.expiry_date BETWEEN CURRENT_DATE AND (CURRENT_DATE + INTERVAL
'90 days')
  AND i.status = 'active'
