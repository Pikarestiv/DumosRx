      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            onClick={() =>
              toast.info("Status Page Pending", {
                description:
                  "The external monitoring dashboard is being provisioned.",
              })
            }
          >
            System Status
        ...
        <Card className="border-none bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Badge
                variant="outline"
                className="text-emerald-500 border-emerald-500/20 font-bold"
              >
                High Performance
              </Badge>
            </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <Card className="border-none shadow-sm bg-white dark:bg-slate-900">
          <CardHeader>
            ...
            <CardDescription>
              Real-time resource utilization across the cluster
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-black text-indigo-500">
                  {systemHealth?.resources?.cpu || 0}%
                </span>
              </div>
              <Progress
                value={systemHealth?.resources?.cpu || 0}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-black text-emerald-500">
                  {systemHealth?.resources?.memory?.used} /{" "}
                  {systemHealth?.resources?.memory?.total}
                </span>
              </div>
              <Progress
                value={systemHealth?.resources?.memory?.percent || 0}
                className="h-2 bg-slate-100 dark:bg-slate-800"
              />
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-black text-amber-500">
                  {systemHealth?.resources?.disk?.used} /{" "}
                  {systemHealth?.resources?.disk?.total}
                </span>
              </div>
              <Progress
                value={systemHealth?.resources?.disk?.percent || 0}
                className="h-2 bg-slate-100 dark:bg-slate-800"
              />
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-black text-indigo-500">
                  {systemHealth?.resources?.database?.load || 0}%
                </span>
              </div>
              <Progress
                value={systemHealth?.resources?.database?.load || 0}
                className="h-2 bg-slate-100 dark:bg-slate-800"
              />
            </div>
          </CardContent>
        ...
        <Card className="border-none shadow-sm bg-white dark:bg-slate-900">
          <CardHeader>
            ...
            <CardDescription>
              Status of global API gateways and edge locations
            </CardDescription>
          </CardHeader>
