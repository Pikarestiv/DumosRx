  ...
  const fetchData = async () => {
    ...
    try {
      const [invRes, lowRes, expRes, valRes] = await Promise.all([
        webApiClient.getStockBatch(storeIdStr, 1, 100),
        webApiClient.getLowStockStockBatch(storeIdStr),
        webApiClient.getExpiringStockBatch(storeIdStr, expiryDays),
        webApiClient.getStockBatchValue(storeIdStr),
      ]);
      setStockBatch(invRes?.data || invRes || []);
      setLowStock(lowRes || []);
      setExpiring(expRes || []);
